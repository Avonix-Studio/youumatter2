...
(function () {
  ...
  function startAlpine() {
    if (
      window.Alpine &&
      typeof window.Alpine.start === "function" &&
      !window.__yum2AlpineStarted
    ) {
      ...
      init: function () {
        ...
        if (el) {
          this._scroll = function () {
            ...
            self.page = Math.max(
              0,
              Math.min(self.pages - 1, Math.round(el.scrollLeft / w)),
            );
          };
        ...
        if ("IntersectionObserver" in window && this.items.length) {
          ...
          this._observer = new IntersectionObserver(
            function (entries) {
              ...
              if (visible[0]) {
                var idx = self.items.findIndex(function (it) {
                  return it.id === visible[0].target.id;
                });
  ...
  window.yum2.copyLink = function (btn) {
    var value =
      (btn && btn.getAttribute && btn.getAttribute("data-yum2-copy")) ||
