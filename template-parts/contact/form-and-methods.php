<?php
/**
 * Contact: "Send a note" form section. Wraps WPForms shortcode in
 * the youumatter2 design system card. The form itself is styled
 * via CSS overrides in tailwind.src.css.
 *
 * Falls back to WhatsApp if WPForms plugin is not active.
 *
 * @package youumatter2
 */
...
// WPForms shortcode ID – set via Customizer or hardcode here.
$form_id = (int) get_theme_mod( 'yum2_contact_form_id', 1053 );
$has_wpforms = function_exists( 'wpforms' ) && $form_id > 0;
?>
	...
	<div class="max-w-6xl mx-auto grid grid-cols-1 lg:grid-cols-[1.35fr_1fr] gap-10 lg:gap-14">
		...
		<?php if ( $has_wpforms ) : ?>
			<div class="bg-[#f8f3e9] border border-forest/15 rounded-[24px] p-6 md:p-10 shadow-[0_24px_60px_-30px_rgba(26,58,25,0.2)]">
				...
				<p class="text-forest/45 mt-5" style="font-size:12px;line-height:1.6;">
					<?php esc_html_e( 'Your message is private – we\'ll respond within 24 hours on weekdays.', 'youumatter2' ); ?>
				</p>
			</div>
		<?php else : ?>
			<?php // Fallback: WhatsApp card (same as original methods.php left column) ?>
			<div class="bg-[#f8f3e9] border border-forest/15 rounded-[24px] p-6 md:p-10 shadow-[0_24px_60px_-30px_rgba(26,58,25,0.2)]">
				<p class="text-terracotta tracking-[2px] uppercase mb-3" style="font-size:11px;font-weight:600;">
